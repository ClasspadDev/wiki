...
- `dy` is the **row index**, that starts at 0 and goes up to `DVD_H - 1` (in our case, 15).
- `dx` is the **column index**, that starts at 0 and goes up to `DVD_W - 1` (in our case, 22).
...
![](/wiki/img/python/examples/from-text.svg)
