...
However, downspreading degrades performance and creates major issues at higher PLL values. The TMU-based timer operates at Pϕ/4, and spread spectrum causes timer drift. Furthermore, the slowdown depends on the PLL value, estimated by:

$$ f_{SS} = \begin{cases} f_0 \times \left(1 - 1.6\% \times \frac{PLL}{16}\right), & \text{if } 3 \le PLL \le 32 \\ f_0, & \text{if } 1 \le PLL \le 2 \end{cases} $$
